    ...
    do
    {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        else
        {
          ...
          if ( min_ellip_dist2_ <= pow((swarm_clearance_ * 1.25), 2) )
          {
            cout << "debug:" << ros::Time::now() 
              << " my_p=" << debug_.my_p.transpose() 
              << " swarm_id=" << debug_.swarm_id 
              << " swarm_p=" << debug_.swarm_p.transpose()
              << " swarm_dist=" << debug_.swarm_dist
              << " traj_id=" << debug_.traj_id
              << " start_time=" << debug_.start_time - ros::Time::now().toSec()
              << " dura=" << debug_.duration
              << endl;
          }
                                         ...
                                         double &costp)
  {
    ...
    for (size_t id = 0; id < swarm_trajs_->size(); id++)
    {
      ...
      if (min_ellip_dist2_ > ellip_dist2)
      {
        ...
        debug_.my_p = p;
        debug_.swarm_dist = sqrt(ellip_dist2);
        debug_.swarm_id = id;
        debug_.swarm_p = swarm_p;
        debug_.traj_id = swarm_trajs_->at(id).traj_id;
        debug_.start_time = swarm_trajs_->at(id).start_time;
        debug_.duration = swarm_trajs_->at(id).duration;
      }
    }
